-- Add validation to compute_assessment_scores function to ensure user_id matches authenticated context
-- This prevents potential unauthorized nudge creation via the SECURITY DEFINER function
...
BEGIN
  ...
  -- Add explicit validation: only create nudges if assessment belongs to authenticated user
  IF TG_OP = 'INSERT' THEN
    -- Validate that NEW.user_id matches the user creating the assessment
    -- This prevents unauthorized nudge creation even with SECURITY DEFINER
