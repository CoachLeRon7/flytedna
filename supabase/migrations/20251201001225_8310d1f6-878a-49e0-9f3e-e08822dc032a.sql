-- Update validate_and_consume_pilot_code to notify admins
...
BEGIN
  ...
  SELECT first_name, last_name, email INTO user_profile
  FROM profiles
  WHERE id = _user_id;

  -- Notify admins via http request to edge function
  PERFORM net.http_post(
    url := current_setting('app.settings.supabase_url') || '/functions/v1/notify-admin-events',
